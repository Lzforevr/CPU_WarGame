...
import { useFieldState, Notification, Form, Button, InputGroup } from '@douyinfe/semi-ui'
...
const SendCaptchaButton = (props) => {
    ...
    const sendCaptcha = async () => {
        ...
        if(!emailState.value.match(/^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+$/)) {
            // 检查邮箱格式是否符合:邮箱@域名
            Notification.error({
                title: '邮箱格式不正确！',
                duration: 2
        ...
        fetch(serverUrl + '/user/send_captcha', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({email: emailState.value})
        }).then(async response => {
    ...
    // 继承Button的属性
    return (
        <Button disabled={isSending} type="primary" theme='solid' style={{width: '110px', marginLeft: '10px'}} onClick={sendCaptcha}>{isSending ? `${waitTime}s后重新获取` : "获取验证码"}</Button>
    )
}
